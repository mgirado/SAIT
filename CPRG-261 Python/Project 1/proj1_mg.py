...
current_time = get_time.strftime(
    "%Y-%m-%d_%H-%M-%S"
)  # Formats output of current time to YYYY-MM-DD_HH-MM-SS
temp_folder_name = "/pyscript_"  # Set directory name that holds temporary/output files.
temp_folder = (
    "/home/" + current_user + temp_folder_name + current_time
)  # Assigns desired directory for output.
os.mkdir(temp_folder)  # Uses os module to create the directory.


desired_name = input(  # Prompts user to assign filename, can be left empty for default filename.
    "Enter desired file name without file extension to output data to or leave empty for default: "
)

# Code checks if input was empty, if empty it'll use default filename, otherwise use user inputted string.
if desired_name == "":
    desired_name = "output.txt"
    print("Default chosen, file will be named: " + desired_name)
else:
    desired_name = desired_name + ".txt"
    print("File will be named: " + desired_name)
...
def get_cpu_info():  # Function that tries to read /proc/cpuinfo and output it to a file if successful.
    ...
    try:
        ...
        print('Could not "/proc/cpuinfo" file, stopping script.')
        sys.exit(0)
...
try:
    ...
    print('Could not find "' + temp_passwd + '", stopping script.')
    sys.exit(0)
...
# Tries to open file at services_list, otherwise informs user that it could not be found.
try:
    services_file = open(services_list, "r")
except FileNotFoundError:
    print('Could not find "' + services_list + '", stopping script.')
...
# Tells user data was taken from (hostname) on (date & time) and sends it to master output file.
print(
    "Data taken from " + socket.gethostname() + " machine on: " + current_time,
    file=master_output,
)
...
# Tells user in terminal that the script was successfully run and that the data got sent to (desired location).
print("Script successfully run, data outputted to " + master_file)